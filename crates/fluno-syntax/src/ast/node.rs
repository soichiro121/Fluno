...
impl Span {
    ...
    pub fn union(&self, other: &Span) -> Self {
        if self.line == other.line {
            let col = std::cmp::min(self.column, other.column);
            let end = std::cmp::max(self.column + self.length, other.column + other.length);
            Span::new(self.line, col, end.saturating_sub(col))
        } else if self.line < other.line {
            Span::new(self.line, self.column, 0) // Cross-line approximation
        } else {
            Span::new(other.line, other.column, 0)
        }
    }
}
...
pub struct TraitDef {
    pub name: Identifier,
    pub type_params: Vec<TypeParameter>,
    pub assoc_types: Vec<AssociatedTypeDecl>,
...
impl fmt::Display for Type {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Type::EstimatorKind => write!(f, "EstimatorKind"),
            Type::SurrogateRule => write!(f, "SurrogateRule"),
