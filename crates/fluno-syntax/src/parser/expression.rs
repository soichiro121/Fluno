...
impl<'a> Parser<'a> {
                ...
                if self.check(TokenKind::Question) {
                    ...
                    if self.check(TokenKind::Dot) {
                        return Err(ParseError::InvalidOperator {
                            message: "`?.` is not supported in this language mode; use `match` to handle Option/Result explicitly".into(),
    ...
    fn parse_range_expression(&mut self, left: Expression) -> ParseResult<Expression> {
        ...
        let span = if let Some(end_expr) = &end {
            start_span.union(&end_expr.span())
        } else {
            start_span
        };
    ...
    fn parse_prefix(&mut self) -> ParseResult<Expression> {
        ...
        match &token.kind {
            ...
            TokenKind::Stream => self.parse_stream_expression(),
    ...
    fn parse_stream_expression(&mut self) -> ParseResult<Expression> {
        let start_token = self.current.clone();
        let start_span = Span::new(start_token.line, start_token.column, 6);

        let _ = self.advance();
        let body = self.parse_block()?;
        let span = start_span.union(&body.span);

        Ok(Expression::Stream { body, span })
    }
    ...
    fn parse_call_expression(&mut self, callee: Expression) -> ParseResult<Expression> {
        if let Expression::Variable { name, span } = &callee {
            match name.last_name().unwrap_or("") {
                "Some" => {
                    let _ = self.advance();
