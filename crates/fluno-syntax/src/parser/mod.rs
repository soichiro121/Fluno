...
enum ParsedTraitItem {
    AssocType(AssociatedTypeDecl),
    Method(TraitMethod),
}
...
impl<'a> Parser<'a> {
    ...
    fn parse_trait(&mut self, _attributes: Vec<Attribute>) -> ParseResult<TraitDef> {
        ...
        while !self.check(TokenKind::RBrace) && !self.is_at_end() {
            match self.parse_trait_item()? {
                ParsedTraitItem::AssocType(assoc) => assoc_types.push(assoc),
                ParsedTraitItem::Method(method) => methods.push(method),
            }
        }
        self.expect(TokenKind::RBrace)?;
    ...
    fn parse_trait_item(&mut self) -> ParseResult<ParsedTraitItem> {
        if self.match_any(&[TokenKind::Type]) {
            let name_token = self.expect(TokenKind::Identifier)?;
            self.expect(TokenKind::Semicolon)?;
            return Ok(ParsedTraitItem::AssocType(AssociatedTypeDecl {
                name: Identifier::new(
                    name_token.text.unwrap_or_default(),
                    Span::new(name_token.line, name_token.column, 0),
                ),
        ...
        self.expect(TokenKind::Semicolon)?;
        Ok(ParsedTraitItem::Method(TraitMethod {
            name,
            params,
            return_type,
            span: Span::initial(),
    ...
    pub fn parse_type(&mut self) -> ParseResult<Type> {
        match self.current.kind {
            TokenKind::Dyn => {
                let dyn_token = self.expect(TokenKind::Dyn)?;
                let trait_start = Span::new(dyn_token.line, dyn_token.column, 0);
                ...
                if self.is_reserved_type_name_path(&trait_path) {
                    return Err(ParseError::InvalidSyntax {
                        message: "dyn must be followed by a trait path (e.g., dyn foo::Bar)"
                            .to_string(),
                        line: self.current.line,
                        column: self.current.column,
                    });
                ...
                if path.segments.len() == 1 {
                    if let PathSeg::Ident(id) = &path.segments[0] {
                        match id.name.as_str() {
                            ...
                            "EstimatorKind" => return Ok(Type::EstimatorKind),
                            "SurrogateRule" => return Ok(Type::SurrogateRule),
    ...
    fn parse_let_statement(&mut self) -> ParseResult<Statement> {
        ...
        let _is_mut = self.match_any(&[TokenKind::Mut]);
    ...
    fn _parse_pattern(&mut self) -> ParseResult<Pattern> {
        ...
        match &self.current.kind {
            ...
            TokenKind::LParen => {
                ...
                let first_pattern = self._parse_pattern()?;
...
mod tests {
    ...
    #[test]
    fn test_parse_stream_expression() {
        let input = "fn main() { stream { let x: Int = 1; } }";
        let result = parse(input);
        assert!(result.is_ok());
        ...
        if let Item::Function(func) = &prog.items[0] {
            ...
            match &func.body.statements[0] {
                Statement::Expression(Expression::Stream { body, span }) => {
                    assert_eq!(body.statements.len(), 1);
                    match &body.statements[0] {
                        Statement::Let { .. } => {}
                        _ => panic!("Expected let statement inside stream block"),
                    }
                    assert!(span.length > 6);
                }
                _ => panic!("Expected stream expression statement"),
            }
        } else {
    ...
    fn test_parse_tuple_expression() {
        let input = "fn main() { let x = (1, true); }";
        let result = parse(input).expect("parser should accept tuple expression");
        let Item::Function(func) = &result.items[0] else {
            panic!("expected function item");
        };
        let Statement::Let {
            init: Some(Expression::Tuple { elements, .. }),
            ..
        } = &func.body.statements[0]
        else {
            panic!("expected tuple literal in let initializer");
        };
        assert_eq!(elements.len(), 2);
    }

    #[test]
    fn test_parse_single_element_tuple_type_annotation() {
        let input = "fn main() { let x: (Int,) = (1,); }";
        let result = parse(input).expect("parser should accept single element tuple type");
        let Item::Function(func) = &result.items[0] else {
            panic!("expected function item");
        };
        let Statement::Let { ty: Some(ty), .. } = &func.body.statements[0] else {
    ...
    #[test]
    fn test_parse_future_type_annotation() {
        let input = "fn main() { let f: Future<Int> = spawn 1; }";
        let result = parse(input).expect("parser should accept Future type annotation");
        let Item::Function(func) = &result.items[0] else {
            panic!("expected function item");
        };
        let Statement::Let { ty: Some(ty), .. } = &func.body.statements[0] else {
        ...
        let Type::Future(inner) = ty else {
            panic!("expected Future type");
        };
        assert_eq!(inner.as_ref(), &Type::Int);
    }

    #[test]
    fn test_parse_parenthesized_pattern_not_tuple() {
        let input = "fn main() { let (x) = 1; }";
        let result = parse(input).expect("parser should accept parenthesized pattern");
        let Item::Function(func) = &result.items[0] else {
            panic!("expected function item");
        };
        let Statement::Let { pattern, .. } = &func.body.statements[0] else {
            panic!("expected let statement");
        };
        match pattern {
            Pattern::Identifier { name, .. } => assert_eq!(name.name, "x"),
            _ => panic!("expected grouped identifier pattern"),
        }
    }

    #[test]
    fn test_parse_try_operator_expression() {
        let input = "fn main() { let x = Ok(1)?; }";
        let result = parse(input).expect("parser should accept Result try operator");
        let Item::Function(func) = &result.items[0] else {
            panic!("expected function item");
        };
        let Statement::Let {
            init: Some(Expression::Try { .. }),
            ..
        } = &func.body.statements[0]
        else {
            panic!("expected try expression in let initializer");
        };
    }

    #[test]
    fn test_parse_try_dot_is_rejected() {
        let input = "fn main() { let x = [1, 2, 3]?.len(); }";
        let result = parse(input);
        assert!(result.is_err());
    }

    #[test]
    fn test_parse_trait_associated_type_declaration() {
        let input = r#"
            trait IteratorLike {
                type Item;
        ...
        "#;
        let result = parse(input).expect("parser should accept trait associated type declaration");

        let Item::Trait(trait_def) = &result.items[0] else {
    ...
    #[test]
    fn test_parse_let_mut_statement_with_mut_token() {
        let input = "fn main() { let mut value: Int = 1; }";
        let result = parse(input).expect("parser should accept let mut statement");
        let Item::Function(func) = &result.items[0] else {
            panic!("expected function item");
        };
        let Statement::Let { pattern, .. } = &func.body.statements[0] else {
    ...
    #[test]
    fn test_parse_for_statement_uses_in_token() {
        let input = "fn main() { for x in xs {} }";
        let result = parse(input).expect("parser should accept for-in statement");
        let Item::Function(func) = &result.items[0] else {
            panic!("expected function item");
        };
        let Statement::For {
        ...
        let Expression::Variable {
            ...
            panic!("expected variable iterator expression");
        };
        let PathSeg::Ident(iter_ident) = &iter_name.segments[0] else {
            panic!("expected identifier path segment");
        };
        assert_eq!(iter_ident.name, "xs");
    }

    #[test]
    fn test_parse_for_statement_rejects_missing_in_keyword() {
        let input = "fn main() { for x xs {} }";
        assert!(parse(input).is_err());
    }

    #[test]
    fn test_parse_wildcard_pattern_uses_underscore_token() {
        let input = "fn main() { let _ = 1; }";
        let result = parse(input).expect("parser should accept wildcard pattern");
        let Item::Function(func) = &result.items[0] else {
            panic!("expected function item");
        };
        let Statement::Let { pattern, .. } = &func.body.statements[0] else {
    ...
    #[test]
    fn test_parse_dyn_trait_type_with_dyn_token() {
        let input = "fn main() { let value: dyn Displayable = x; }";
        let result = parse(input).expect("parser should accept dyn trait type");
        let Item::Function(func) = &result.items[0] else {
            panic!("expected function item");
        };
        let Statement::Let { ty: Some(ty), .. } = &func.body.statements[0] else {
        ...
        let Type::DynTrait { trait_path } = ty else {
            panic!("expected dyn trait type");
        };
        let PathSeg::Ident(trait_name) = &trait_path.segments[0] else {
            panic!("expected trait path identifier");
        };
        assert_eq!(trait_name.name, "Displayable");
    }

    #[test]
    fn test_parse_dyn_trait_rejects_reserved_type_name() {
        let input = "fn main() { let value: dyn Int = x; }";
        assert!(parse(input).is_err());
    }

    #[test]
    fn test_parse_dyn_trait_cast_expression() {
        let input = "fn main() { let value = x as dyn Displayable; }";
        let result = parse(input).expect("parser should accept dyn trait cast");
        let Item::Function(func) = &result.items[0] else {
            panic!("expected function item");
        };
        let Statement::Let {
            init: Some(Expression::Cast { target_type, .. }),
            ..
        } = &func.body.statements[0]
        else {
            panic!("expected cast initializer");
        };
        let Type::DynTrait { trait_path } = target_type else {
            panic!("expected dyn trait cast target");
        };
        let PathSeg::Ident(trait_name) = &trait_path.segments[0] else {
            panic!("expected trait path identifier");
        };
