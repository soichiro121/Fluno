...
mod tests {
    ...
    #[test]
    fn test_keywords() {
        let input = "fn let if else while for return stream";
        ...
        assert_eq!(tokens[7].kind, TokenKind::Stream);
    }

    #[test]
    fn test_promoted_keyword_and_category_tokens() {
        let input = "in mut dyn _ _name";
        let tokens = tokenize(input).unwrap();

        assert_eq!(tokens[0].kind, TokenKind::In);
        assert_eq!(tokens[1].kind, TokenKind::Mut);
        assert_eq!(tokens[2].kind, TokenKind::Dyn);
        assert_eq!(tokens[3].kind, TokenKind::Underscore);
        assert_eq!(tokens[3].text(), Some("_"));
        assert_eq!(tokens[4].kind, TokenKind::Identifier);
        assert_eq!(tokens[4].text(), Some("_name"));
    }
